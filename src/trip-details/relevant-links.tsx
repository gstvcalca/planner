...
export function RelevantLinks({openAddLinkModal, tripInfo}: RelevantLinksProps){
    ...
    return (
        <div className="space-y-6">
            ...
            {tripInfo?.infos.map((info) => {
                return (
                    <div key={tripInfo._id + info.title} className="flex justify-between">
            ...
            {tripInfo?.links.map((link) => {
                return(
                    <div key={tripInfo?._id + link.title} className="flex items-center justify-between gap-4"> {/* links list */}
